...
function createNewConfig(config, req) {
  ...
  indexArray.forEach(function(element, index) {
    if (req['printerIP' + element]) { // in the form
      newConfig.greenpiList[element].printerIP = req['printerIP' + element];
    } else { // from another pi
      newConfig.greenpiList[element].printerIP = req.greenpiList[index].printerIP;
    }

    if (req['greenpiIP' + element]) { // in the form
      newConfig.greenpiList[element].greenpiIP = req['greenpiIP' + element];
    } else { // from another pi
      newConfig.greenpiList[element].printerIP = req.greenpiList[index].greenpiIP;
    }
  ...
  console.log('NEW CONFIG TO WRITE:')
...
function getIPofOtherPis() {
  var otherIPs = [];
  var i = 0;

  for (i = 0; i <  config.greenpiList.length; i++) {
    if (i !== config.thispiIndex) {
      otherIPs.push(config.greenpiList[i].greenpiIP);
    }
  }
  return otherIPs;
}

function updateOtherPi(request, ip) {
...
exports.updateOtherPi = updateOtherPi;
