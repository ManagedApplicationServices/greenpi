...
function createNewConfig(config, req) {
  ...
  indexArray.forEach(function(element) {
    ...
    if (newConfig.greenpiList[element].thispi === 'yes') {
...
function updateAllPi(request, otherIPs, callback) {
  ...
  indexArray.forEach(function(element) {
    body.greenpiList['printerIP' + element] = body['printerIP' + element];
    delete body['printerIP' + element];

    body.greenpiList['greenpiIP' + element] = body['greenpiIP' + element];
