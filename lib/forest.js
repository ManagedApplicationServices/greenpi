...
function initForest(forest, isReset, config) {
  var totalPrinterNum = config.greenpiList.length;

  forest.completePrinterURL = 'http://' + config.greenpiList[config.thispiIndex].printerIP + config.paperUsagePath;
  forest.maxPaperToPrint = parseInt(config.paperUsageCap, 10) / totalPrinterNum;
  forest.interval = parseInt(config.interval, 10);
  forest.printerIP = config.greenpiList[config.thispiIndex].printerIP;
  forest.printerInfoUrl = 'http://' + config.greenpiList[config.thispiIndex].printerIP + config.machineDetailPath;
...
function getArrayIndex() {
  var indexArray = [];
  var length = require('../config').greenpiList.length;
  var i = 0;
