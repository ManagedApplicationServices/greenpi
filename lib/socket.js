...
module.exports.listen = function(app, logger) {
  ...
  function readConfigFile(callback) {
    fs.readFile(configFile, 'utf8', function (err, data) {
      if (err) {logger.error('Error: ' + err); return; }
    ...
    function () {
      ...
      function startSimulation() {
        ...
        function printLog(callback) {
          logger.info(forest.count + ':Date: ' + dateFormat(now, 'dddd, mmmm dS, yyyy, h:MM:ss TT'));
          logger.info(forest.count + ':LIVE count: ' + forest.livePrinterCount);
          logger.info(forest.count + ':OFFSET count: ' + forest.offset);
          logger.info(forest.count + ':Yearly CAP: ' + forest.maxPaperToPrint + ', Monthly CAP: ' + parseInt(forest.maxPaperToPrint)/12);
          logger.info(forest.count + ':Remaining yearly CAP: ' + forest.remainingPaper);
          logger.info(forest.count + ':Leaf difference count: ' + forest.leafDifference);
          logger.info('---------------------------------');
          callback(null, forest);
        }
