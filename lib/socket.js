...
module.exports.listen = function(app, logger) {
  ...
  function readConfigFile(callback) {
    ...
    forest = configFile.initForest(forest, config);
    callback(null, forest);
  }
  ...
  async.series(
    ...
    function() {
      ...
      function startSimulation() {
        var currentMonth = month.getCurrentMonthIndex(),
          currentMonthYear = month.getCurrentMonthYear(),
          leafMonthTotal = 0;
        ...
        function isInternetAvailable(callback) {
          isOnline([ forest.printerIP ], function(err, online) {
            if (online) {
              forest.isInternetAvailable = true;
              callback(null, forest);
            } else {
              if (forest.demo) {
                logger.info(timestamp.get() + 'Demo - no connection to printer: ' + forest.printerIP);
              } else {
                logger.error(timestamp.get() + 'No connection to the printer: ' + forest.printerIP);
              }
