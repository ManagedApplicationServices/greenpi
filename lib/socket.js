...
module.exports.listen = function(app, logger) {
  ...
  function resetDB(callback) {
    db.reset(client, resetParam, function() {
      callback(null);
    });
  }
  ...
  function readConfigFile(callback) {
    if (resetParam === 'reset') {
      forest = simulationLib.initForest(forest, true, require('../config'));
    } else {
      forest = simulationLib.initForest(forest, false, require('../config'));
    }
    callback(null, forest);
  }
  ...
  function getMonthAndData(callback) {
    db.getMonthAndData(client, resetParam);
    callback(null);
  }
    ...
    function() {
      ...
      socket.on('reset', function() {
        ...
        async.series([
          ...
          function(next) {
            db.getMonthAndData(client, 'reset');
            next(null);
          },
